...
public class ClusterConfigSyncThread implements Runnable {
    ...
    private void syncAvailableClusters() {
        List<com.ymm.redis.admin.entity.Cluster> availableClusters = clusterService.getAllAvailableClusters();
        if (availableClusters != null && availableClusters.size() > 0) {
            for (com.ymm.redis.admin.entity.Cluster cluster : availableClusters) {
    ...
    private void syncUnAvailableClusters() {
        List<com.ymm.redis.admin.entity.Cluster> unAvailableClusters = clusterService.getAllUnAvailableClusters();
        if (unAvailableClusters != null && unAvailableClusters.size() > 0) {
            for (com.ymm.redis.admin.entity.Cluster cluster : unAvailableClusters) {
