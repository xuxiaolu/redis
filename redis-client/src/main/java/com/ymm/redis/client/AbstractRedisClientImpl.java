...
public abstract class AbstractRedisClientImpl implements RedisClient {
    ...
    private <T> T execute(CacheKey cacheKey, boolean isUse, RedisClient.CommandType commandType, Protocol.Command command, Executor<T> executor) {
        Transaction t = Cat.newTransaction(commandType.getType(), command.name());
        try {
            String finalKey = "";
            if (isUse) {
            ...
            T result = executor.execute(finalKey);
            t.setStatus(Message.SUCCESS);
            return result;
        } catch (Throwable e) {
            LOGGER.error("{} invoke error", command.name());
            t.setStatus(e);
            throw e;
        } finally {
            t.complete();
    ...
    @Override
    public Long lInsert(CacheKey key, final Position position, final String pivot, final String value) {
    ...
    protected abstract Long dolInsert(String finalKey, Position position, String pivot, String value);
